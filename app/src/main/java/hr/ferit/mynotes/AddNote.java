...
import hr.ferit.mynotes.Note;
import hr.ferit.mynotes.R;
import hr.ferit.mynotes.SimpleDatabase;
...
public class AddNote extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //toolbar.setTitleTextColor(getResources().getColor(R.color.));
        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setTitle("Nova bilješka");

        noteDetails = findViewById(R.id.noteDetails);
       // noteTitle = findViewById(R.id.noteTitle);

       // notetitletChangedListener(new TextWatcher()
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if(item.getItemId() == R.id.save){
            if(noteDetails.getText().length() != 0){
                Note note = new Note(noteDetails.getText().toString(),todaysDate,currentTime);
